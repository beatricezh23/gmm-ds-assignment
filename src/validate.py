...
"""from src.checks import detect_abnormal_production, HistoricalData"""
from src.lib.events import ProductionParserEvent


CHECKS_TO_RUN = [verify_total_production_above_zero, detect_missing_production_data]
...
def is_event_valid(event: ProductionParserEvent) -> tuple[bool, list[str]]:
    ...
    for mode in production_modes:
        if not detect_abnormal_production(event, mode, historical_data):
...
def validate_events(events: list[ProductionParserEvent]) -> list[tuple[datetime, str]]:
    ...
    #historical_data.add_event(event)
